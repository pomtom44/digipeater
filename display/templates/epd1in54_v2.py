...
def draw_table_page(driver, title, headers, rows):
    """Same layout as default.py, larger fonts for this smaller, higher-DPI panel."""
    return default.draw_table_page(driver, title, headers, rows, title_size=20, header_size=14, value_size=15, row_height=26)


def draw_symbol_page(driver, title, symbol_image, comment):
    """Same layout as default.py, larger fonts for this smaller, higher-DPI panel."""
    return default.draw_symbol_page(driver, title, symbol_image, comment, title_size=20, comment_size=15)


def draw_station_page(driver, title, symbol_image, callsign, lat, lon, comment):
    """Same layout as default.py, larger fonts for this smaller, higher-DPI panel."""
    return default.draw_station_page(
        driver, title, symbol_image, callsign, lat, lon, comment, title_size=20, call_size=22, text_size=15,
    )
...
def draw_status_page(driver, title: str, rows: list[tuple[str, str]]):
    ...
    title_font = fit_font(draw, title, FONT_BOLD, content_width, 23)
