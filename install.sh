...
# Internal: spinner wrapper around a command, output captured to $_SPIN_LOG; returns the command's exit status.
_spin() {
    local msg="$1"; shift
    _SPIN_LOG=$(mktemp)
    "$@" > "$_SPIN_LOG" 2>&1 &
...
# Runs a command behind a spinner, only showing output on failure; aborts the installer on failure.
run_with_spinner() {
    local msg="$1"
    if ! _spin "$@"; then
...
# ── Prime sudo ─────────────────────────────────
# Priming sudo up front avoids a backgrounded spinner hitting a blocked password prompt.
...
# Explicitly enables gpsd's default auto-detect mode; services/gpsconfig.py switches it to a fixed device later if needed.
...
# Separate sudoers rule per binary, scoped narrowly like nmcli's above.
...
# Root-only GPS/chrony/timezone actions live in this one script, scoped like nmcli/reboot above.
...
# Scoped to reading only the direwolf unit's journal, covering both the error-log modal (services/system.py) and packet_log.py's tail.
...
# --system-site-packages so the venv can see apt-installed RPi.GPIO/spidev (pip versions are unreliable for these).
...
# Static binary from GitHub releases; services/tiles.py uses it to fetch OSM data via Protomaps, per OSM's offline-tile-use policy. Soft/non-fatal.
...
# Fonts/sprites MapLibre needs to render local map data, vendored from Protomaps. Soft/non-fatal.
...
# Coarse ~1GB whole-world basemap so the Map caching wizard step always has something to show; skipped if go-pmtiles didn't install.
...
# CAP_NET_BIND_SERVICE for port 80; no CapabilityBoundingSet (it broke this service's sudo nmcli calls for hotspot management).
...
# Not enabled here; main.py starts/stops it based on config.yaml. Restart=/RestartSec= here are just install-time fallbacks, overridden by services/restart_policy.py's drop-in.
...
# Off by default (config.yaml's map.auto_update); timer runs every 15min and reads config fresh each time rather than being tied to a fixed schedule (scripts/auto_tile_update.py).
...
# ── WiFi country ───────────────────────────────
# Unset WiFi country soft-blocks the radio via rfkill; checked via rfkill directly since raspi-config's own getter returns a non-empty sentinel even when unset.
...
# Asked here (not in the web wizard) since the display is needed at first boot, before any wizard exists; options are read live from display/waveshare/'s registry.
